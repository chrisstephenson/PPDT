...
(define-type fonk-def
  [fonk-def-1 (f-ismi : symbol) (f-formal-param : (listof symbol)) (f-vucut : arith)])

(define-type arith
  [arith-num (n : number)]
  [arith-symbol (s : symbol)]
  [arith-fonk-uygula (f-ismi : symbol) (f-gerçek-param : (listof arith))]
...
(define-type subst
  [subst-çift (ikame-edilecek : symbol) (ikame-eden : arith)])
    ...
    [(s-exp-list? s)
     (let ([sl (s-exp->list s)])
       (case (s-exp->symbol (first sl))
         ...
         [else (arith-fonk-uygula (s-exp->symbol (first sl)) (map parse-arith (rest sl)))]))]
...
(define (parse-fonk-def [s : s-expression]) : fonk-def
  (cond
    [(s-exp-list? s)
     (let ([sl (s-exp->list s)])
       (case (s-exp->symbol (first sl))
         [(fonk) (fonk-def-1 (s-exp->symbol (second sl)) (map s-exp->symbol (s-exp->list (third sl))) (parse-arith (fourth sl)))]
...
(define (fonk-uygula-eval  [fl : (listof fonk-def)] [f : fonk-def] [gerçek-param : (listof arith)]) : number
  (type-case fonk-def f
    [fonk-def-1 (isim formal-param vucut) (eval fl (foldr ikame-çift vucut (map2 subst-çift formal-param gerçek-param)))]))

(define (ikame-çift [çift : subst] [ifade : arith]) : arith
  (type-case subst çift
    [subst-çift (ikame-edilecek ikame-eden) (ikame ifade ikame-edilecek ikame-eden)]))
...
(define (ikame [orijinal : arith] [ikame-edilecek : symbol] [ikame-eden : arith])
  (type-case arith orijinal
    ...
    [arith-fonk-uygula (f-name f-param)
                       (arith-fonk-uygula f-name (map (λ (p) (ikame p ikame-edilecek ikame-eden)) f-param))]))
...
(fonk fib-iter (a b n) (eğer-sıfır-ya-eksiyse n a (fib-iter b (+ a b) (+ n -1)))))
                               (fib-iter 0 1 25))))

(eval-program (parse-program '((
                                (fonk test-cap (x y) (+ x y)))
                               (test-cap 3 (+ x 2)))))